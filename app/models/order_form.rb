class OrderForm
  ...
  with_options presence: true do  
    validates :user_id, presence: { message: "Token can't be blank" } 
    validates :item_id, presence: { message: "Token can't be blank" }
    ...
    validates :phone_number, format: { with: /\A\d{10,11}\z/, message: "Phone number is invalid. Input only number" }, length: { minimum: 10, message: "Phone number is too short" }
  end
